...
так напрямую через REST API, см. раздел [Установка](#установка).
...
- Возможность работы **в единой сессии**, с сохранением предыдущего контекста.  
...
- `[ Definition ]` открывает в редакторе файл **определения** активной конфигурации (\*.lua.cfg),
  в частности там легко изменить набор доступных параметров, если возникнет такая необходимость.
...
- `[ Models - F6 ]` запрашивает список **моделей** через API, если это предусмотрено конфигурацией,
  ...
  Альтернативно: список моделей может заносится в историю самого поля ввода.
...
- По кнопке `Alt+F4` - быстрый доступ к файлу с **определением** той или иной конфигурации (\*.lua.cfg).
...
- [aichat](https://github.com/sigoden/aichat), [chatgpt-cli](https://github.com/kardolus/chatgpt-cli), [mods](https://github.com/charmbracelet/mods), [sgpt](https://github.com/tbckr/sgpt) - работают с [OpenAI API](https://platform.openai.com/docs/api-reference) (и совместимыми).
...
На данный наиболее сильные модели это `gpt-4o` от [OpenAI](https://platform.openai.com/), `gemini-1.5-pro` от [Google](https://ai.google/gemini-ecosystem),
...
- Другие *OpenAI*-совместимые: [Mistral AI](https://mistral.ai/), [AI21 labs](https://www.ai21.com) *Jamba*.  
  К этой же категории относятся целый ряд провайдеров, предоставляющие доступ к OpenSource
...
- На некоторых сервисах только часть моделей доступно бесплатно:
  ...
  - [openrouter](https://openrouter.ai/models?q=:free) (бесплатно в основном небольшие модели, но доступны и такие как `lfm-40b` и `hermes-3-llama-3.1-405b`)
...
- Некоторые сервисы позволяют бесплатно пользоваться только сайтом, но их при желании тоже можно использовать, указав подходящие данные в заголовках:
...
- `keepass` позволяет держать свои собственные пресеты в базе [KeePass](https://keepass.info/).  
  Аналогичным образом можно организовать работу и с другими аналогичными утилитами.
...
По причине их неопределённого статуса пользоваться ими следует с осторожностью, и не передавать важные данные!
