...
- Верхний раздел списка содержит “нативные” конфигурации, работающие с сетью средствами Lua.  
  ...
  *OpenAI-compatible*, *Cohere*, *Google*: **Gemini**, **Gemini CLI API**, **Antigravity API**.
...
- Дополнительно: в директории `extra` есть конфигурации для ещё нескольких утилит, таких как [Gemini CLI](https://geminicli.com/), [Copilot CLI](https://github.com/features/copilot/cli), …  
...
- Другие *OpenAI*-совместимые: [Mistral AI](https://mistral.ai/), [xAI](https://x.ai/grok) *Grok*, и несметное количество других провайдеров.
- Провайдеры с собственным API, такие как [Google](https://ai.google.dev/) *Gemini*, [Anthropic](https://www.anthropic.com/) *Claude*, [Cohere](https://cohere.com/command) *Command*,
  требуют поддержки посредством соответствующей “нативной” конфигурации.  
...
В комплекте идут следующие нативные конфигурации:

- **OpenAI-compatible**  
  В комплекте поставки идут готовые **пресеты** для множества совместимых сервисов,
  подробнее они рассматриваются в следующем разделе.
- **Cohere**: бесплатно 1,000 запросов в месяц.
- **Google Gemini**: бесплатный доступ к моделям уровня до `gemini-3-flash-preview`.  
  *Free-tier* лимиты непостоянны, и сейчас они у меня например составляют (для каждой модели отдельно)
  по 20 запросов в день.
  - **Gemini CLI API**: на основе внутреннего API [Gemini CLI](https://geminicli.com/); логин через боаузер.  
    Предоставляется бесплатный доступ к моделям уровня до `gemini-3-pro-preview`.  
    Дневная квота: 1000 запросов.
  - **Antigravity API**: использует внутренние API *Google* [Antigravity](https://antigravity.google/); логин через боаузер.  
    Предоставляется бесплатный доступ к моделям уровня до `gemini-3-pro-high`,
    а также `claude-sonnet-4-5`/`claude-opus-4-5-thinking` от [Anthropic](https://www.anthropic.com/).  
    Лимиты изменчивы, и конкретных цифр не указано, но в любом случае больше чем
    при использовании **Google Gemini** Free-tier.
...
- Для использования других надо зарегистрироваться и получить ключ.
  ...
  - [qwen-code-api](https://github.com/QwenLM/qwen-code) (`qwen3-coder-plus`, `qwen3-vl-plus`):
    Бесплатно 2000 запросов в день.  
    Авторизация через браузер.
