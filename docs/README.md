...
- Также можно работать через [сторонние утилиты](#меню-выбора-конфигурации):
  ...
  - В `%PATH%` должны быть **утилиты**, соответствующие имеющимся файлам **определений** (\*.lua.cfg),
    см. раздел [Обзор утилит](#меню-выбора-конфигурации).
...
- Не требуют ключа (`apikey`) только эти пресеты:
  ...
  - `anyai`: (OpenSource модели небольшого размера): контекст лимитирован
